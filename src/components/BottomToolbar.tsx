...
export function BottomToolbar(props: {
  ...
  const buttonClassByFreeHand = (freeHand: boolean) => {
    const baseClass = "w-12 h-full rounded flex justify-center items-center group relative";
    const activeClass = freeHand ? "bg-blue-500 text-white rounded" : "";
    return baseClass + " " + activeClass;
  }
      ...
      <div className="w-full h-full flex justify-between px-4">
        <div className="w-full h-full flex justify-between">
          <button onClick={() => onDrawTypeChange(DrawType.None)} className={buttonClassByDrawType(DrawType.None)}>
            ...
            <span className="w-auto absolute hidden group-hover:flex -left-0 -top-0 -translate-y-full px-2 py-2 bg-gray-600 rounded-lg text-center text-white text-sm">Move</span>
          </button>
          <button onClick={() => onDrawTypeChange(DrawType.Line)} className={buttonClassByDrawType(DrawType.Line)}>
            ...
            <span className="w-auto absolute hidden group-hover:flex -left-0 -top-0 -translate-y-full px-2 py-2 bg-gray-600 rounded-lg text-center text-white text-sm">Line</span>
          </button>
          <button onClick={() => onDrawTypeChange(DrawType.Polygon)} className={buttonClassByDrawType(DrawType.Polygon)}>
            ...
            <span className="w-auto absolute hidden group-hover:flex -left-0 -top-0 -translate-y-full px-2 py-2 bg-gray-600 rounded-lg text-center text-white text-sm">Polygon</span>
          </button>
          <button onClick={() => onDrawTypeChange(DrawType.Rectangle)} className={buttonClassByDrawType(DrawType.Rectangle)}>
            ...
            <span className="w-auto absolute hidden group-hover:flex -left-0 -top-0 -translate-y-full px-2 py-2 bg-gray-600 rounded-lg text-center text-white text-sm">Rectangle</span>
          </button>
          <button onClick={onFreeHandToggle} className={buttonClassByFreeHand(freeHand)}>
            ...
            <span className="w-24 absolute hidden group-hover:flex -left-0 -top-0 -translate-y-full px-2 py-2 bg-gray-600 rounded-lg text-center text-white text-sm">Free hand mode</span>
          </button>
        </div>
